...
export async function buildSuggestionForItem(
  ...
  const item = await getItemWithEnrichment(env.DB, id);
  ...
  if (env.OPENAI_API_KEY) {
    try {
      return await generateLlmEnrichment(
        item,
        env.OPENAI_API_KEY,
  ...
  return generateHeuristicEnrichment(item);
}

export async function persistSuggestionAndApply(
  env: Env,
  id: number,
  suggestion: Awaited<ReturnType<typeof buildSuggestionForItem>>
) {
...
export async function enrichAndApplyItem(env: Env, id: number) {
  const suggestion = await buildSuggestionForItem(env, id);
  await persistSuggestionAndApply(env, id, suggestion);
  return getItemWithEnrichment(env.DB, id);
}
